 ...
 */
void CoalGeneration::coalesceOneGeneration(double new_total_popn) {
 ...
 */
void CoalGeneration::coalesceLiteOneGeneration(double new_total_popn, int max_coalescent){
    const auto& weights = this->number_of_descendants_distribution;
 ...
 */
void CoalGeneration::updateSample() {
 ...
 */
void CoalGeneration::incrementAlleleFrequencySpectrumMutations(
 ...
 */
void CoalGeneration::incrementAlleleFrequencySpectrumFoundingDiversity(
    ...
    double log_pnonzero = log(pnonzero);
    double log_pzero = log(pzero);
    for (int i=1;i<=max_ic;++i) {
        for (int j=i;j<=max_ic;++j) {
            double pij = exp(log_pnonzero*i + log_pzero *(j-i) + lgamma(j+1) -lgamma(i+1) -lgamma(j-i+1));
...
vector<double> CoalGeneration::calculateAFS(const double sample_popn,
    ...
    if (selection != 0.0) { printf("Forward method only works with selection==0.0 (is %lf)", selection); exit(0); }

    const int max_coalescent = 10;

    CoalGeneration generation (popn_history[0], sample_popn);
